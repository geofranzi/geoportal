...
class MapViewerSerializer(serializers.ModelSerializer):
    class Meta:
        model = MapViewer
        fields = (
            'id', 'title', 'center_lat', 'center_lon', 'center_proj', 'map_proj', 'map_resolutions', 'zoom_min', 'zoom_max', 'zoom_init', 'auth_registration',
            'addexternallayer', 'time_slider', 'time_slider_start', 'time_slider_end', 'time_slider_interval', 'time_slider_dates', 'search_url', 'html_info',
            'html_footer')
...
class MapViewerDetail(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        if mapviewer.auth_perm:
            if not request.user.is_authenticated():
                auth_view = False
                error_msg = 'Please log in to use this viewer!'
            elif request.user not in mapviewer.auth_users.all() and len(
                    set(list(request.user.groups.all())) & set(list(mapviewer.auth_groups.all()))) == 0 and request.user.is_superuser is not True:
                auth_view = False
                error_msg = 'You do not have rights to use this viewer!'

        download_perm = True
        if mapviewer.download_perm:
            if not request.user.is_authenticated():
                download_perm = False
            elif request.user not in mapviewer.auth_users.all() and len(
                    set(list(request.user.groups.all())) & set(list(mapviewer.auth_groups.all()))) == 0 and request.user.is_superuser is not True:
        ...
        if serializer.data['time_slider'] is True and (serializer.data['time_slider_dates'] is None or serializer.data['time_slider_dates'] == ''):
            ...
            if serializer.data['time_slider_end'] is None:
        ...
        for group in layergroups:
            ...
            for inline in layerObjects:
                ...
                # check permission
                if inline.layer.auth_perm:
                    if not request.user.is_authenticated():
                        layerauth = True
                        continue
                    elif request.user not in inline.layer.auth_users.all() and len(
                            set(list(request.user.groups.all())) & set(list(inline.layer.auth_groups.all()))) == 0 and request.user.is_superuser is not True:
                        continue
        ...
        for inlineBase in LayerBaseInline.objects.filter(mapviewer_id=pk).order_by('order'):
            ...
            layerdata = layer.data
            if baselayer.wmts_projection is not None:
