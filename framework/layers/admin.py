...
class LayersForm(forms.ModelForm):
    def clean(self):
        ...
        # downloads
        if cleaned_data.get('downloadable') is True and not (cleaned_data.get('download_url') != '' or cleaned_data.get('download_file') is not None):
...
class LayersAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            ...
            'fields': ('identifier', 'title', 'title_en', 'abstract', 'abstract_en', 'topicCategory', 'scope', 'meta_contact', 'meta_contacts', 'meta_language',
                       'meta_characterset', 'meta_date', 'publishable')
        }),
        (None, {
            'classes': ('suit-tab', 'suit-tab-ogc',),
            'fields': ('ogc_link', 'ogc_layer', 'ogc_type', 'ogc_imageformat', 'ogc_attribution', 'statistic', 'ogc_getfeatureinfo', 'ogc_time', 'ogc_times',
                       'legend_graphic', 'legend_url', 'legend_colors', 'legend_info', 'min_zoom', 'max_zoom', 'sos_default_field', 'meta_file_info')
        }),
        (None, {
        ...
        (None, {
            ...
            'fields': ('dataset_contact_new', 'point_of_contacts', 'date_creation', 'date_publication', 'date_revision', 'progress', 'language', 'characterset',
                       'dataset_epsg', 'format', 'meta_lineage', 'data_source')
        }),
        (None, {
    ...
    save_as = True
    suit_form_tabs = (
        ('general', 'General'), ('ogc', 'Data'), ('wmts', 'WMTS'), ('dataset', 'Dataset'), ('location', 'Location'), ('temporalextent', 'Temporal Extent'),
        ('spatialresolution', 'Spatial Resolution'), ('onlineresources', 'Online Resources'), ('keyword', 'Keyword'),
        ('conformity_constraints', 'Conformity / Constraints'), ('download', 'Download'), ('permissions', 'Permissions'))
